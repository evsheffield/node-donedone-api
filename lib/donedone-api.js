 ...
 * @param  {Function} callback           Callback function with two arguments `(err, data)`
 */
exports.createFullReleaseBuild = function(subdomain, username, passwordOrAPIToken, id, data, callback) {
  async.waterfall([
    ...
    function(releaseBuildInfo, cb) {
      data.order_numbers = releaseBuildInfo.order_numbers_ready_for_next_release.join(',');
    ...
    function(peopleInProject, cb) {
      data.user_ids_to_cc = _.pluck(peopleInProject, 'id').join(',');
      exports.createReleaseBuildForProject(subdomain, username, passwordOrAPIToken, id, data, callback);
    }
  ]);
}
